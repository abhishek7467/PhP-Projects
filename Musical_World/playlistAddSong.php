...
border: 3px red ;
    ...
    /* Add any other styles you want to reset or change */
}
.song_playlist {
    /* Text properties */
...
<body>
    ...
    <a class="song_playlist" id="add_song" href="playlist.php">Your Playlist</a>
 ...
 <?php while ($song = mysqli_fetch_assoc($sql_res)) {
        ...
        <ol class="list-group list-group-numbered">
            <li class="list-group-item d-flex justify-content-between align-items-start">
                ...
                <span class="badge bg-primary rounded-pill">
                    <button class="add-button" data-item-id="<?php echo $ID; ?>">
                        <div id="response<?php echo $ID; ?>">Add</div>
                    </button>
    ...
    document.querySelectorAll('.add-button').forEach(function(button) {
        button.addEventListener('click', function() {
            var Song_id = this.getAttribute('data-item-id');
            AddSong(this, Song_id);
        });
    });

    function AddSong(button, Song_id) {
        // Make an AJAX request to remove the item with its ID
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'addToPlayList.php', true);
        ...
        xhr.send('Song_id=' + Song_id); // Send the AJAX request with the item's ID
    }
</script>
